...
bool copy_uvm(uint64_t d_map, uint64_t s_map, int size)
{
    bool status = false;
    unsigned int index;
    PD pd = NULL;
    ...
    if (page != NULL) {
        memset(page, 0, PAGE_SIZE);
        status = map_pages(d_map, 0x400000, 0x400000 + PAGE_SIZE, V2P(page), PTE_P|PTE_W|PTE_U);

        if (status == true) {
            pd = find_pdpt_entry(s_map, 0x400000, 0, 0);
            if (pd == NULL) {
                free_vm(d_map);
                return false;
            }
