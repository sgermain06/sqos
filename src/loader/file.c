...
{
    struct BPB *p = get_fs_bpb();
    uint32_t offset = (p->reserved_sector_count + (uint32_t)p->fat_count * p->sectors_per_fat) * p->bytes_per_sector;
...
int load_file(char *path, uint64_t addr)
{
    ...
    if (index != 0xffffffff) {
        ...
        uint32_t file_size = dir_entry[index].file_size;
        uint32_t cluster_index = dir_entry[index].cluster_index;
